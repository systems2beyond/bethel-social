                                    ...
                                    {msg.role === 'model' ? (
                                        <>
                                            ...
                                            <div
                                                ...
                                                onClickCapture={(e) => {
                                                    // DEBUG LOGS
                                                    console.log("AiNotesModal: Capture Click detected", e.target);
                                                    const target = e.target as HTMLElement;
                                                    const verseLink = target.closest('.verse-link');
                                                    if (verseLink) {
                                                        console.log("AiNotesModal: Verse link found", verseLink);
                                                        e.stopPropagation();
                                                        e.preventDefault();

                                                        let ref = verseLink.getAttribute('data-verse');
                                                        const href = verseLink.getAttribute('href');
                                                        console.log("AiNotesModal: Ref/Href", { ref, href });
                                                        ...
                                                        if (ref) {
                                                            // Robust Regex (Same as BibleAiChatModal)
                                                            const match = ref.trim().match(/(.+?)\s(\d+)(?::(\d+)(?:-(\d+))?)?$/);
                                                            console.log("AiNotesModal: Match result", match);
                                                            ...
                                                            if (match) {
                                                                ...
                                                                console.warn("AiNotesModal: Regex failed for", ref);
                                                                openBible(undefined, true); // Fallback to new tab anyway? Or same tab? User wants new tab.
                                                            }
                                                        }
                                                    }
                                                }}
