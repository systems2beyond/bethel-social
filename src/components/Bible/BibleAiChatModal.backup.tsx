...
function AiMessageContent({ content, openBible, onClose }: { content: string, openBible: any, onClose: () => void }) {
    ...
    useEffect(() => {
        ...
        const handleClick = (e: MouseEvent) => {
            ...
            if (verseLink) {
                console.log("BibleAiChatModal: Native Click Capture", verseLink);
                ...
                if (ref) {
                    // Blur active element to prevent keyboard from popping up if user was typing
                    ...
                    if (match) {
                        ...
                        console.warn("BibleAiChatModal: Regex failed", ref);
        ...
        const handleTouchStart = (e: TouchEvent) => {
            // Record start time and position to distinguishing tap from scroll
        ...
        const handleTouchEnd = (e: TouchEvent) => {
            ...
            const startTime = parseFloat(container.dataset.touchStartTime || '0');
            const touch = e.changedTouches[0];
            ...
            // Thresholds for a "tap": move < 30px, duration < 500ms
            if (dist < 30 && duration < 500) {
                console.log("BibleAiChatModal: Manual TAP detected");

                // Construct a fake click target check
                ...
                const verseLink = target.closest('.verse-link');
                if (verseLink) {
                    console.log("BibleAiChatModal: TAP on Verse Link!", verseLink);
        ...
        // Keep Click for Desktop, but maybe rely on tap for mobile?
        // Actually, let's keep click capture for desktop, but add touch logic for mobile.
        // If touchend handles it, we preventDefault, so click should not fire.

        container.addEventListener('click', handleClick, true);
        container.addEventListener('touchstart', handleTouchStart, { passive: true }); // Passive for scroll perf
        container.addEventListener('touchend', handleTouchEnd, { passive: false }); // Non-passive to allow preventDefault
                ...
                <div className="p-4 bg-white dark:bg-zinc-900 border-t border-gray-100 dark:border-zinc-800">
                    <form onSubmit={(e) => handleSendMessage(e)} className="relative">
                        <input
                            ...
                            className="w-full pl-4 pr-10 py-3 bg-gray-100 dark:bg-zinc-800 rounded-full border-none focus:ring-2 focus:ring-purple-500 text-base sm:text-sm"
                            autoFocus
                        />
                        <button
                            type="submit"
