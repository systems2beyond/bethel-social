...
export default function BibleStudyModal({ onClose }: BibleStudyModalProps) {
    ...
    useEffect(() => {
        console.log('[BibleStudyModal] collaborationId changed:', collaborationId);
    }, [collaborationId]);
    ...
    const handleOpenShareModal = () => {
        // BUG FIX: Prioritize existing session, Implicit Fellowship, or active note ID
        const activeTab = tabs.find(t => t.id === activeTabId);
        const fallbackFellowshipId = activeTab
            ? `fellowship-${activeTab.reference.book}-${activeTab.reference.chapter}`
            : null;

        if (collaborationId) {
            setGeneratedShareId(collaborationId);
        } else if (rightPaneView === 'fellowship' && fallbackFellowshipId) {
            // Upgrade Implicit Session to Explicit
            setGeneratedShareId(fallbackFellowshipId);
        } else if (activeNoteId) {
            setGeneratedShareId(activeNoteId);
        } else {
            setGeneratedShareId(`note-${user?.uid}-${Date.now()}`);
        }
    ...
    // START: Top-level handler for Fellowship Scroll Joining (Fixes Hook Violation)
    const handleJoinFellowshipScroll = useCallback((newId: string, title?: string) => {
        openCollaboration(newId, title);
    }, [openCollaboration]);

    const handleSaveNotes = async (content: string, titleOverride?: string) => {
        if (!user) return;
        // Debug Log
        // console.log('BibleStudyModal: handleSaveNotes called. collaborationId:', collaborationId);
        ...
        if (collaborationId) {
            console.log('BibleStudyModal: Skipped save due to collaboration.');
            return;
        }
    ...
    const handleAskAi = useCallback((prompt: string, autoSend: boolean = true) => {
        setAiQuery(prompt);
