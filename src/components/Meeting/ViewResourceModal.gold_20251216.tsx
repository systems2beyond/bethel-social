...
export function ViewResourceModal({ isOpen, onClose, title, content, type = 'scroll', meetingId }: ViewResourceModalProps) {
    ...
    const handleAddToNotes = async () => {
        ...
        try {
            ...
            // Auto-Tabs Logic: Workbook Style
            // RECURSIVE DOM TRAVERSAL (Strict Visual Order)

            const refs: any[] = [];
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = content;

            // Regex for Plain Text (same as before)
            const textRegex = /\b((?:1|2|3|I|II|III)?\s?[A-Z][a-z]+(?: [A-Z][a-z]+)*)\s+(\d+):(\d+)(?:-(\d+))?\b/g;

            // Helper to parse verse:// URI
            const parseVerseUri = (uri: string): any | null => {
                const refStr = decodeURIComponent(uri.replace('verse://', ''));
                // Support full "Book Chapter:Verse-End" or simple "Book Chapter"
                const refMatch = refStr.match(/(.+?)\s(\d+)(?::(\d+)(?:-(\d+))?)?$/);
                if (refMatch) {
                    return {
                        book: refMatch[1].trim(),
                        chapter: parseInt(refMatch[2]),
                        verse: refMatch[3] ? parseInt(refMatch[3]) : undefined,
            ...
            const traverse = (node: Node) => {
                if (node.nodeType === Node.ELEMENT_NODE) {
                    ...
                    // Case 1: Loop/Link with verse://
                    // Check if it's an anchor with verse href
                    if (el.tagName === 'A' && el.getAttribute('href')?.startsWith('verse://')) {
                        const href = el.getAttribute('href');
                        if (href) {
                            const ref = parseVerseUri(href);
                            if (ref) refs.push(ref);
                        }
                        // Do NOT traverse children of a link to avoid double-counting text inside it
                        return;
                    }
                }
            ...
            // Deduplicate (Keep FIRST occurrence in list)
            ...
            if (uniqueRefs.length > 0) {
                // 2. Create a Tab Group (Neon Colors)
                const neonColors = ['#06b6d4', '#ec4899', '#8b5cf6', '#10b981', '#f59e0b'];
                const randomColor = neonColors[Math.floor(Math.random() * neonColors.length)];

                const groupId = createTabGroup(title, randomColor);

                // 3. Open tabs in that group (Ordered as found)
                openMultipleTabs(uniqueRefs, groupId);
            }
