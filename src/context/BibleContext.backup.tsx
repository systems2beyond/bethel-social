...
export function BibleProvider({ children }: { children: ReactNode }) {
    ...
    const openBible = useCallback((ref?: BibleReference, newTab = false) => {
        ...
        // Only open the Quick Modal if Study Mode is NOT open
        // If Study Mode IS open, the user will see the change there (since it consumes the same tabs)
        if (!isStudyOpen) {
            setIsOpen(true);
        }
    }, [activeTabId, isStudyOpen]);
